...
Модуль и аргумент числителя и знаменателя:
\begin{align}
  |\text{числитель}| &= \sqrt{41{,}70^2 + 43{,}16^2} = 60{,}10\ \text{Ом}\\
  \arg(\text{числитель}) &= \arctan\left(\frac{43{,}16}{41{,}70}\right) = 45{,}98^\circ\\
...
Закон Кирхгофа выполняется с высокой точностью.

\subsection*{8. Баланс мощностей}

\textbf{Активная мощность в сопротивлениях:}
\begin{align}
...
\textbf{Реактивная мощность в индуктивностях и ёмкостях:}
\begin{align}
...
Для элементов $L_2$ и $C_2$, включённых параллельно, необходимо рассчитать токи через них отдельно:

\begin{align}
  I_{L2} &= \frac{U_{BC}}{X_{L2}} = \frac{64{,}14}{5{,}341} = 12{,}01\ \text{А}\\
  I_{C2} &= \frac{U_{BC}}{X_{C2}} = \frac{64{,}14}{3{,}183} = 20{,}15\ \text{А}
\end{align}

\begin{align}
  Q_{L2} &= I_{L2}^2 X_{L2} = 12{,}01^2 \cdot 5{,}341 = 770\ \text{вар}\\
  Q_{C2} &= -I_{C2}^2 X_{C2} = -20{,}15^2 \cdot 3{,}183 = -1292\ \text{вар}
\end{align}

\textbf{Суммарная реактивная мощность:}
\begin{equation}
  \sum Q = 1421 - 1200 + 770 - 1292 + 718 - 650 = -233\ \text{вар}
\end{equation}
...
\textbf{Расчётная полная мощность:}
\begin{equation}
  S_{\text{расч}} = \sqrt{P^2 + Q^2} = \sqrt{3792^2 + (-233)^2} = 3799\ \text{ВА}
\end{equation}

Погрешность баланса: $\Delta = \frac{|3821 - 3799|}{3821} \cdot 100\% = 0{,}58\%$ — баланс мощностей выполняется.
...
\begin{equation}
  C_{\text{рез}} = \frac{1}{\omega X_{C,\text{рез}}} = \frac{1}{314{,}16 \cdot 1{,}710} = 1{,}86\ \text{мФ} = 1860\ \text{мкФ}
\end{equation}

При добавлении конденсатора ёмкостью $C_{\text{рез}} = 1860$ мкФ последовательно в неразветвлённую часть цепи наступит резонанс напряжений.

В режиме резонанса:
\begin{itemize}
  \item[$-$] Общее сопротивление станет чисто активным: $Z_{\text{рез}} = 12{,}557$ Ом;
  \item[$-$] Ток возрастёт до: $I_{\text{рез}} = \frac{220}{12{,}557} = 17{,}52$ А;
  \item[$-$] Коэффициент мощности станет равным единице: $\cos\varphi = 1$;
  \item[$-$] Реактивная мощность источника будет равна нулю.
\end{itemize}

\section*{Сравнение с результатами моделирования}

\subsection*{Результаты из Electronics Workbench:}

\begin{table}[H]
\centering
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Параметр} & \textbf{Расчёт} & \textbf{EWB} & \textbf{Погрешность, \%} \\
\hline
$I_1$, А & 17,37 & 17,75 & 2,19 \\
$U_{BC}$, В & 64,14 & 46,66 & 27,3 \\
$I_2$, А & 5,363 & 5,027 & 6,26 \\
$I_3$, А & 12,78 & 9,331 & 27,0 \\
\hline
\end{tabular}
\caption*{Таблица 1 — Сравнение расчётных и измеренных значений}
\end{table}

\subsection*{Анализ расхождений:}

Наблюдается существенное расхождение по напряжению $U_{BC}$ и току $I_3$. Возможные причины:

\begin{enumerate}
  \item Неточность параметров элементов в симуляторе;
  \item Влияние внутреннего сопротивления измерительных приборов;
  \item Возможная ошибка в топологии подключения элементов $L_2$ и $C_2$;
  \item Округление значений параметров при вводе в EWB.
\end{enumerate}

Проверка по закону Кирхгофа для данных симуляции:
\begin{equation}
  I_1 = I_2 + I_3 \Rightarrow 17{,}75 \ne 5{,}027 + 9{,}331 = 14{,}36\ \text{А}
\end{equation}

Несоответствие указывает на необходимость уточнения параметров моделирования.

\textbf{Общие выводы по моделированию:}

Общий ток $I_1$ совпадает с расчётом с высокой точностью (погрешность 2,19\%), что подтверждает правильность определения общего сопротивления цепи. Расхождения в токах ветвей и напряжении на параллельном участке требуют дополнительной проверки схемы в симуляторе.
...
\begin{itemize}
  ...
  \item[$-$] Определены действующие значения токов во всех ветвях цепи: $I_1 = 17{,}37$ А, $I_2 = 5{,}363$ А, $I_3 = 12{,}78$ А;
  \item[$-$] Записаны выражения для мгновенных значений токов и напряжения на параллельном участке;
  \item[$-$] Выполнена проверка расчётов по первому закону Кирхгофа с высокой точностью;
  \item[$-$] Составлен баланс мощностей: активная мощность $P = 3792$ Вт, реактивная мощность $Q = -233$ вар, полная мощность $S = 3821$ ВА, коэффициент мощности $\cos\varphi = 0{,}992$;
  \item[$-$] Определены условия резонанса напряжений: для достижения резонанса необходимо добавить конденсатор ёмкостью $C_{\text{рез}} = 1860$ мкФ в неразветвлённую часть цепи;
  \item[$-$] Проведено моделирование в Electronics Workbench, результаты которого подтверждают расчёт общего тока с погрешностью 2,19\%;
  \item[$-$] Получены практические навыки анализа цепей переменного тока и работы с системой схемотехнического моделирования.
\end{itemize}

Методика символического метода расчёта продемонстрировала свою эффективность для анализа сложных цепей синусоидального тока. Баланс мощностей выполняется с погрешностью менее 1\%, что свидетельствует о корректности проведённых вычислений.
