...
MainWindow::MainWindow(Database *db, int user_id, bool is_admin)
    : db_(db), current_user_id_(user_id), is_admin_(is_admin) {
...
void MainWindow::initialize_main_window() {
  if (is_admin_) {
    ...
    connect(users_search_edit_, &QLineEdit::returnPressed, this,
            &MainWindow::search_users);
    users_filter_layout->addWidget(users_search_edit_, 1);
    auto users_clear_btn = new QPushButton("×");
    users_clear_btn->setMaximumWidth(30);
    connect(users_clear_btn, &QPushButton::clicked, this,
            &MainWindow::clear_users_search);
    auto users_search_btn = new QPushButton("Поиск");
    users_search_btn->setMaximumWidth(100);
    connect(users_search_btn, &QPushButton::clicked, this,
            &MainWindow::search_users);
    ...
    users_table_->horizontalHeader()->setSectionResizeMode(
        QHeaderView::Stretch);
    ...
    connect(refresh_users_btn, &QPushButton::clicked, this,
            &MainWindow::refresh_users_table);
    ...
    hubs_search_edit_->setPlaceholderText(
        "Поиск по названию, местоположению, серийному номеру или владельцу...");
    connect(hubs_search_edit_, &QLineEdit::returnPressed, this,
            &MainWindow::search_hubs);
    hubs_filter_layout->addWidget(hubs_search_edit_, 1);
    auto hubs_clear_btn = new QPushButton("×");
    hubs_clear_btn->setMaximumWidth(30);
    connect(hubs_clear_btn, &QPushButton::clicked, this,
            &MainWindow::clear_hubs_search);
    auto hubs_search_btn = new QPushButton("Поиск");
    hubs_search_btn->setMaximumWidth(100);
    connect(hubs_search_btn, &QPushButton::clicked, this,
            &MainWindow::search_hubs);
    ...
    hubs_table_->setHorizontalHeaderLabels(
        {"ID", "Владелец", "Название", "Местоположение", "Серийный номер"});
    ...
    connect(add_hub_btn, &QPushButton::clicked, this,
            &MainWindow::add_admin_hub);
    ...
    connect(delete_hub_btn, &QPushButton::clicked, this,
            &MainWindow::delete_hub);
    auto refresh_hubs_btn = new QPushButton("Обновить");
    connect(refresh_hubs_btn, &QPushButton::clicked, this,
            &MainWindow::refresh_hubs_table);
    ...
    devices_search_edit_->setPlaceholderText(
        "Поиск по названию, типу или хабу...");
    connect(devices_search_edit_, &QLineEdit::returnPressed, this,
            &MainWindow::search_devices);
    ...
    connect(devices_clear_btn, &QPushButton::clicked, this,
            &MainWindow::clear_devices_search);
    auto devices_search_btn = new QPushButton("Поиск");
    devices_search_btn->setMaximumWidth(100);
    connect(devices_search_btn, &QPushButton::clicked, this,
            &MainWindow::search_devices);
    ...
    devices_table_->setHorizontalHeaderLabels(
        {"ID", "Хаб", "Название", "Тип", "Статус"});
    devices_table_->horizontalHeader()->setSectionResizeMode(
        QHeaderView::Stretch);
    ...
    connect(add_device_btn, &QPushButton::clicked, this,
            &MainWindow::add_admin_device);
    auto edit_device_btn = new QPushButton("Редактировать");
    connect(edit_device_btn, &QPushButton::clicked, this,
            &MainWindow::edit_admin_device);
    auto delete_device_btn = new QPushButton("Удалить");
    connect(delete_device_btn, &QPushButton::clicked, this,
            &MainWindow::delete_admin_device);
    auto refresh_devices_btn = new QPushButton("Обновить");
    connect(refresh_devices_btn, &QPushButton::clicked, this,
            &MainWindow::refresh_devices_table);
    ...
    connect(profile_action, &QAction::triggered, this,
            &MainWindow::edit_profile);
    auto logout_action = user_menu->addAction("Выйти");
    connect(logout_action, &QAction::triggered, this, &MainWindow::logout);
    ...
    connect(manage_hubs_action, &QAction::triggered, this,
            &MainWindow::manage_hubs);
...
void MainWindow::add_hub() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    if (!res)
      return;
    try {
      int hub_id = db_->create_hub(current_user_id_, res->name, res->location,
                                   res->serial_number);
      QMessageBox::information(this, "Успех",
                               QString("Хаб '%1' добавлен с ID: %2")
                                   .arg(QString::fromStdString(res->name))
                                   .arg(hub_id));
    } catch (const std::exception &e) {
      QMessageBox::critical(
          this, "Ошибка", QString("Не удалось добавить хаб: %1").arg(e.what()));
    }
  }
}
...
void MainWindow::refresh_admin_data() {
  try {
    ...
    for (int r = 0; r < (int)users.size(); ++r) {
      ...
      users_table_->setItem(
          r, 1, new QTableWidgetItem(QString::fromStdString(username)));
      users_table_->setItem(
          r, 2, new QTableWidgetItem(QString::fromStdString(email)));
      users_table_->setItem(r, 3,
                            new QTableWidgetItem(is_admin ? "Да" : "Нет"));
    }
    ...
    for (int r = 0; r < (int)hubs.size(); ++r) {
      ...
      hubs_table_->setItem(
          r, 1, new QTableWidgetItem(QString::fromStdString(username)));
      hubs_table_->setItem(r, 2,
                           new QTableWidgetItem(QString::fromStdString(name)));
      hubs_table_->setItem(
          r, 3, new QTableWidgetItem(QString::fromStdString(location)));
      hubs_table_->setItem(
          r, 4, new QTableWidgetItem(QString::fromStdString(serial)));
    }
    ...
    for (int r = 0; r < (int)devices.size(); ++r) {
      ...
      devices_table_->setItem(
          r, 1, new QTableWidgetItem(QString::fromStdString(hub_name)));
      devices_table_->setItem(
          r, 2, new QTableWidgetItem(QString::fromStdString(name)));
      devices_table_->setItem(
          r, 3, new QTableWidgetItem(QString::fromStdString(type_name)));
      devices_table_->setItem(
          r, 4, new QTableWidgetItem(QString::fromStdString(status)));
    }
  } catch (const std::exception &e) {
    QMessageBox::critical(
        this, "Ошибка",
...
void MainWindow::add_user() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    try {
      db_->create_user(username.toStdString(), email.toStdString(),
      ...
      QMessageBox::critical(
          this, "Ошибка",
          QString("Не удалось добавить пользователя: %1").arg(e.what()));
    }
  }
}
...
void MainWindow::edit_user() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    try {
      db_->update_user(user_id, username.toStdString(), email.toStdString(),
                       is_admin);
      refresh_admin_data();
      ...
      QMessageBox::critical(
          this, "Ошибка",
          QString("Не удалось обновить пользователя: %1").arg(e.what()));
    }
  }
}
...
void MainWindow::delete_user() {
  ...
  if (reply == QMessageBox::Yes) {
    try {
      ...
      QMessageBox::critical(
          this, "Ошибка",
          QString("Не удалось удалить пользователя: %1").arg(e.what()));
    }
  }
}
...
void MainWindow::add_admin_hub() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    try {
      db_->create_hub(user_id, name.toStdString(), location.toStdString(),
                      serial.toStdString());
      refresh_admin_data();
...
void MainWindow::edit_hub() {
  ...
  int old_user_id =
      hubs_table_->item(row, 1)->text().toInt(); // hidden, but assume
  ...
  auto serial_label =
      new QLabel(old_serial); // Показываем серийный номер как статический текст
  form->addRow("Владелец:", user_combo);
  form->addRow("Название:", name_edit);
  form->addRow("Местоположение:", location_edit);
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    try {
      db_->update_hub(hub_id, user_id, name.toStdString(),
                      location.toStdString(), serial.toStdString());
      refresh_admin_data();
      QMessageBox::information(this, "Успех", "Хаб обновлен");
    } catch (const std::exception &e) {
      QMessageBox::critical(
          this, "Ошибка", QString("Не удалось обновить хаб: %1").arg(e.what()));
    }
  }
}
...
void MainWindow::delete_hub() {
  ...
  auto reply = QMessageBox::question(this, "Подтверждение",
                                     QString("Удалить хаб '%1'?").arg(name));
  if (reply == QMessageBox::Yes) {
    try {
      ...
      QMessageBox::critical(
          this, "Ошибка", QString("Не удалось удалить хаб: %1").arg(e.what()));
    }
  }
}
...
void MainWindow::add_admin_device() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    try {
      db_->save_device(std::nullopt, hub_id, type_id, name.toStdString(),
                       status.toStdString());
      refresh_admin_data();
...
void MainWindow::edit_admin_device() {
  ...
  int old_hub_id = devices_table_->item(row, 1)
                       ->text()
                       .toInt(); // wait, table has hub name, not id
  ...
  QMessageBox::information(this, "Admin",
                           "Edit device - implementation needed");
}
...
void MainWindow::delete_admin_device() {
  ...
  auto reply = QMessageBox::question(
      this, "Подтверждение", QString("Удалить устройство '%1'?").arg(name));
  if (reply == QMessageBox::Yes) {
    try {
...
void MainWindow::refresh_users_table() {
  try {
    ...
    QMessageBox::critical(
        this, "Ошибка",
        QString("Не удалось обновить таблицу пользователей: %1").arg(e.what()));
  }
}
...
void MainWindow::refresh_hubs_table() {
  try {
    ...
    QMessageBox::critical(
        this, "Ошибка",
        QString("Не удалось обновить таблицу хабов: %1").arg(e.what()));
  }
}
...
void MainWindow::refresh_devices_table() {
  try {
    ...
    QMessageBox::critical(
        this, "Ошибка",
        QString("Не удалось обновить таблицу устройств: %1").arg(e.what()));
  }
}
  ...
  try {
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      try {
        // Update user
        if (!password.isEmpty()) {
          std::string new_hash = db_->hash_password_base64(password.toStdString());
          db_->update_user_password(current_user_id_, new_hash);
        }
        db_->update_user(current_user_id_, new_username.toStdString(),
                         new_email.toStdString(), is_admin);
        QMessageBox::information(&dlg, "Успех", "Профиль обновлен");
        setWindowTitle(
            QString::fromStdString("Управление устройствами - Пользователь: " +
                                   new_username.toStdString()));
      } catch (const std::exception &e) {
        QMessageBox::critical(
            &dlg, "Ошибка",
            QString("Не удалось обновить профиль: %1").arg(e.what()));
      }
    }
  } catch (const std::exception &e) {
    QMessageBox::critical(
        this, "Ошибка",
        QString("Не удалось загрузить данные профиля: %1").arg(e.what()));
  }
}
...
void MainWindow::manage_hubs() {
  ...
  table->setHorizontalHeaderLabels(
      {"ID", "Название", "Местоположение", "Серийный номер"});
  ...
  try {
    ...
    for (int r = 0; r < (int)hubs.size(); ++r) {
      ...
      table->setItem(r, 3,
                     new QTableWidgetItem(QString::fromStdString(serial)));
    }
  } catch (const std::exception &e) {
    QMessageBox::critical(
        &dlg, "Ошибка", QString("Не удалось загрузить хабы: %1").arg(e.what()));
    return;
  }
  ...
  connect(edit_btn, &QPushButton::clicked, [&]() {
    ...
    auto edit_buttons =
        new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel);
    connect(edit_buttons, &QDialogButtonBox::accepted, &edit_dlg,
            &QDialog::accept);
    connect(edit_buttons, &QDialogButtonBox::rejected, &edit_dlg,
            &QDialog::reject);
    ...
    if (edit_dlg.exec() == QDialog::Accepted) {
      ...
      try {
        // For simplicity, we'll just update the name. Full implementation would
        ...
        QMessageBox::critical(
            &edit_dlg, "Ошибка",
            QString("Не удалось обновить хаб: %1").arg(e.what()));
      }
    }
  });
  ...
  connect(delete_btn, &QPushButton::clicked, [&]() {
    ...
    auto reply = QMessageBox::question(
        &dlg, "Подтверждение",
        QString(
            "Удалить хаб '%1'? Все устройства в этом хабе также будут удалены.")
            .arg(name));
    if (reply == QMessageBox::Yes) {
      try {
        db_->delete_hub(hub_id);
        ...
        QMessageBox::critical(
            &dlg, "Ошибка",
            QString("Не удалось удалить хаб: %1").arg(e.what()));
      }
    }
  });
...
void MainWindow::search_users() {
  try {
    ...
    auto users = filter.isEmpty()
                     ? db_->get_all_users()
                     : db_->get_users_filtered(filter.toStdString());
    users_table_->setRowCount((int)users.size());
    for (int r = 0; r < (int)users.size(); ++r) {
      auto [id, username, email, is_admin] = users[r];
      users_table_->setItem(r, 0, new QTableWidgetItem(QString::number(id)));
    ...
    QMessageBox::critical(
        this, "Ошибка",
        QString("Не удалось выполнить поиск пользователей: %1").arg(e.what()));
  }
}
...
void MainWindow::search_hubs() {
  try {
    ...
    auto hubs = filter.isEmpty() ? db_->get_all_hubs()
                                 : db_->get_hubs_filtered(filter.toStdString());
    hubs_table_->setRowCount((int)hubs.size());
    for (int r = 0; r < (int)hubs.size(); ++r) {
      auto [id, user_id, username, name, location, serial] = hubs[r];
      hubs_table_->setItem(r, 0, new QTableWidgetItem(QString::number(id)));
    ...
    QMessageBox::critical(
        this, "Ошибка",
        QString("Не удалось выполнить поиск хабов: %1").arg(e.what()));
  }
}
...
void MainWindow::search_devices() {
  try {
    ...
    auto devices = filter.isEmpty()
                       ? db_->get_all_devices()
                       : db_->get_devices_filtered(filter.toStdString());
    devices_table_->setRowCount((int)devices.size());
    for (int r = 0; r < (int)devices.size(); ++r) {
      auto [id, hub_id, hub_name, name, type_name, status] = devices[r];
      devices_table_->setItem(r, 0, new QTableWidgetItem(QString::number(id)));
    ...
    QMessageBox::critical(
        this, "Ошибка",
        QString("Не удалось выполнить поиск устройств: %1").arg(e.what()));
  }
}
